 ...
 * - Connect DHT11 sensor to D3
 * - Connect relay module to D0 (High-Level Trigger)
 * - Connect pump to relay output
 * 
 * Instructions:
...
void setup() {
  ...
  // Check if no test is selected using preprocessor directives
  #if !defined(TEST_SOIL_MOISTURE) && !defined(TEST_DHT11) && !defined(TEST_RELAY) && !defined(TEST_WIFI) && !defined(TEST_MQTT)
...
void setupRelay() {
  ...
  digitalWrite(RELAY_PIN, LOW); // Turn relay OFF initially (HIGH-Level Trigger)
}
#endif
...
void setupMQTT() {
  ...
  // Set to insecure mode for testing purposes
...
void testRelay() {
  ...
  if (currentTime - lastToggleTime > 5000) {
    ...
    // High-level trigger relay, so HIGH turns it ON
 ...
 * 
 * Relay Issues:
 * - Relay not switching: Check if you need to invert the logic (HIGH/LOW)
